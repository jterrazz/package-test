...
case "$COMMAND" in
    spawn)
        ...
        docker run -d \
            ...
            --entrypoint sh \
            busybox:latest \
            -c "echo booting-${NAME} && mkdir -p /workspace && echo hello-from-${NAME} > /workspace/out.txt && sleep 3600" \
            > /dev/null

        echo "spawned ${NAME}"
        ;;
    label)
        # Print the injected test-run id — lets specs assert on the label scope.
        echo "${FAKE_TEST_LABEL:-unset}"
        ;;
    help|*)
