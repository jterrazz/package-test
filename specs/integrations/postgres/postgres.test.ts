...
import { postgres } from '../../../src/integrations/postgres/postgres.js';
...
describe('postgres service', () => {
    ...
    describe('healthcheck', () => {
        ...
        test('fails when no connection string set', async () => {
            // Given - a fresh handle that was never wired
            // Then - the healthcheck explains what is missing
    ...
    describe('query', () => {
        ...
        test('respects column order', async () => {
            // Given - one seeded user, columns requested in reverse order
            await db.reset();
            await db.seed("INSERT INTO \"users\" (name, email) VALUES ('Alice', 'alice@test.com')");
