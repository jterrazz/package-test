<!-- GENERATED by `typescript docs` — DO NOT EDIT -->
# Interface: HttpResult

Defined in: [src/facets/api/api.result.ts:7](https://github.com/jterrazz/package-test/blob/main/src/facets/api/api.result.ts#L7)

Result from an HTTP action (.request(), .get(), .post(), .put(), .delete()).

## Extends

- [`BaseResult`](BaseResult.md)

## Accessors

### response

#### Get Signature

```ts
get response(): ResponseAccessor;
```

Defined in: [src/facets/api/api.result.ts:20](https://github.com/jterrazz/package-test/blob/main/src/facets/api/api.result.ts#L20)

Access the HTTP response (status, headers, body) for assertions.

##### Returns

[`ResponseAccessor`](ResponseAccessor.md)

***

### status

#### Get Signature

```ts
get status(): number;
```

Defined in: [src/facets/api/api.result.ts:25](https://github.com/jterrazz/package-test/blob/main/src/facets/api/api.result.ts#L25)

The HTTP response status code.

##### Returns

`number`

## Methods

### directory()

```ts
directory(path?): DirectoryAccessor;
```

Defined in: [src/core/result/result.ts:55](https://github.com/jterrazz/package-test/blob/main/src/core/result/result.ts#L55)

Access a directory (relative to the working directory) for snapshot assertions.

#### Parameters

| Parameter | Type | Default value |
| ------ | ------ | ------ |
| `path` | `string` | `'.'` |

#### Returns

[`DirectoryAccessor`](DirectoryAccessor.md)

#### Inherited from

[`BaseResult`](BaseResult.md).[`directory`](BaseResult.md#directory)

***

### file()

```ts
file(path): FileAccessor;
```

Defined in: [src/core/result/result.ts:61](https://github.com/jterrazz/package-test/blob/main/src/core/result/result.ts#L61)

Access a single file (relative to the working directory) for content assertions.

#### Parameters

| Parameter | Type |
| ------ | ------ |
| `path` | `string` |

#### Returns

[`FileAccessor`](../type-aliases/FileAccessor.md)

#### Inherited from

[`BaseResult`](BaseResult.md).[`file`](BaseResult.md#file)

***

### table()

```ts
table(tableName, options?): TableAccessor;
```

Defined in: [src/core/result/result.ts:86](https://github.com/jterrazz/package-test/blob/main/src/core/result/result.ts#L86)

Access a database table for row-level assertions via expect() matchers.

#### Parameters

| Parameter | Type |
| ------ | ------ |
| `tableName` | `string` |
| `options?` | \{ `database?`: `string`; \} |
| `options.database?` | `string` |

#### Returns

[`TableAccessor`](TableAccessor.md)

#### Inherited from

[`BaseResult`](BaseResult.md).[`table`](BaseResult.md#table)
