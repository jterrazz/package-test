...
**The `unit` project's budget is the whole suite's floor.** It collects the module tests and the lint suite and runs with no service, no browser and no container, so it is the one project a developer runs on every save: it runs in about ten seconds on a developer machine, and the budget it may not pass is fifteen — measured across runs, because the same tree takes 9 s and 11 s on consecutive ones and a budget set at the best measurement is a budget that fails on noise. The meta-test that proves every rule's reach (`k4-reach-per-path`) runs inside it on ONE oxlint run for the same reason.
