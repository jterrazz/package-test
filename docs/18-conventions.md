# 18 — Conventions: the constitution
...
The **mechanized per-rule catalogue** does not live here — it is **generated from the code** (`src/lint/manifest.ts`, where each rule carries its own normative text) into the [19 — Linting](19-linting.md) catalogue and the agent-facing [`skills/jterrazz-test/references/rules.md`](../skills/jterrazz-test/references/rules.md).
...
The catalogue is organized by family. Each family's usage is illustrated in the chapters; the generated catalogue in [19 — Linting](19-linting.md) carries the normative sentence and channel of every rule.

| Group | Scope                                                                | Explained in                                                          |
| ----- | -------------------------------------------------------------------- | --------------------------------------------------------------------- |
| A     | Runner creation (constructors, services, root)                       | [02](02-developing.md), [10](10-api.md), [17](17-services.md)         |
| B     | Spec chains (setups, terminal actions, Given/Then, `job` vocabulary) | [10](10-api.md), [11](11-jobs.md), [12](12-cli.md)                    |
| C     | Files & folders per feature                                          | [02](02-developing.md), [14](14-assertions.md), [16](16-contracts.md) |
| D     | Assertions, tokens, snapshots, strict contracts                      | [14](14-assertions.md), [15](15-tokens.md), [16](16-contracts.md)     |
| E     | Environment & runner configuration                                   | [02](02-developing.md), [07](07-component.md)                         |
...
| Spec document   | `<case>.spec.yaml`, beside the spec it belongs to — never under `_expected/` ([12](12-cli.md#spec-documents--casespecyaml)) |
| Module test     | `<file>.test.ts`, sibling of `<file>.ts` (under `src/`)                                                                     |
| Component test  | `<file>.test.tsx`, sibling of `<file>.tsx` — or of the `<file>.ts` of a hook or a DOM function ([07](07-component.md))      |
...
2. **A declared service** — `sqlite()`, `postgres()`, `redis()`, `process()` ([17](17-services.md)): real, isolated per worker, reset at the start of every chain.
3. **A contract** — `.intercept()` on a chain (api, jobs, integration, component, website, mobile), `intercept()` in module scope ([16](16-contracts.md)). One queue, three transports, strict from the first contract (rule D7); a streamed body is `http.stream` / `http.sse`.
...
Where a test lives is what makes the rungs mechanizable: a module test may not carry a golden, and a module that needs a real service is an integration spec ([06](06-integration.md)). The reasoning is [ADR-005](decisions/005-the-doubles-ladder-is-closed.md).
...
| D27w unreachable as a negative | a test whose contract list is `http.unreachable()` on every feed and whose assertions name `result.error` alone: the case proves the call FAILED, not that it never happened. Waits on the surface — `http.never(request)` is a product line ([TODO.md](../TODO.md) § What the contract vocabulary still cannot say)                                                                                                                                  |
...
- **Treating these docs as the spec.** They explain and illustrate; this constitution and the generated catalogue ([19 — Linting](19-linting.md)) decide. When you find a discrepancy in the docs, fix the docs in the same change — [02 — Developing](02-developing.md) § What a change owes.
- **Editing the generated catalogue by hand.** The [19 — Linting](19-linting.md) catalogue and `skills/jterrazz-test/references/rules.md` are GENERATED from `src/lint/manifest.ts` — a hand edit is overwritten by the next `npm run docs` and fails the freshness meta-test. Change the rule's text in the code; regenerate.
...
[01 — Architecture](01-architecture.md) · [02 — Developing](02-developing.md) · [03 — Testing](03-testing.md) · [14 — Assertions](14-assertions.md) · [19 — Linting](19-linting.md) · [06 — Integration specs](06-integration.md)
