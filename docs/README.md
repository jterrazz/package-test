...
| Chapter                                     | Holds                                                                                                                                            |
| ------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------ |
| [01 — Architecture](01-architecture.md)     | The runner model behind the six constructors, the four source layers and their edges, the seven enforcement channels, what the tarball publishes |
...
| [03 — Testing](03-testing.md)               | The vitest projects, the spec-tree layout, the end-to-end lint suite, the meta-test channel, goldens and update mode                             |
...
| [17 — Integration specs](17-integration.md) | `specification.integration()`: a module against real services or a golden — `.call()`, `value`/`error`, the `integration()` project              |
...
- **05 to 07 and 14 to 17 follow the facets.** Read 02, then the chapter matching what you specify — API, jobs, CLI, website, mobile, an in-process module, or a rendered component. The six that start something are reached through `specification.*`; a component starts nothing, so it is reached through the `component` chain directly.
