...
- **05 to 07 and 14 to 16 follow the facets.** Read 02, then the chapter matching what you specify — API, jobs, CLI, website, mobile, or a rendered component. The first five start something and are reached through `specification.*`; a component starts nothing, so it is reached through the `component` chain directly.
