...
- **Reaching for a dependency from `core/`.** A new external package belongs in a folder of `seams/` that imports it and nothing else; `core/` importing it directly is the one boundary this package cannot afford to blur, and `i1-layer-boundaries` refuses it.
