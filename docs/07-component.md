...
- **Expecting a name to match a prefix.** It does not since 16.0: a name designates the accessible name WHOLE, so `button('Delete post')` no longer reaches a row's `aria-label="Delete Post all"`. When two elements carry the SAME whole name, the W3 refusal names the accessible name each candidate matched on; read that before scoping, because the landmark it offers is the one that narrows, not necessarily the one you meant.
