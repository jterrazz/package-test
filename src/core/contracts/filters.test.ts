...
describe('matchesText', () => {
    test('an omitted filter matches anything', () => {
        ...
        expect(matchesText(undefined, 'anything at all')).toBeTruthy();
    });
    ...
    test('a string filter is EXACT, never a substring', () => {
        ...
        expect(matchesText(filter, 'Classify the article')).toBeTruthy();
        expect(matchesText(filter, 'Classify the article into TECH')).toBeFalsy();
    });

    test('a RegExp filter tests the observed text', () => {
        // Given - a pattern filter
        // Then - it matches by pattern, not by equality
        expect(matchesText(/Classify/u, 'Please: Classify the article')).toBeTruthy();
        expect(matchesText(/Summarize/u, 'Please: Classify the article')).toBeFalsy();
    });
    ...
    test('match.includes is the explicit containment escape', () => {
        ...
        expect(matchesText(filter, 'Classify the article into TECH')).toBeTruthy();
        expect(matchesText(filter, 'Summarize the article')).toBeFalsy();
    });
    ...
    test('any other match.* matcher goes through the structural engine', () => {
        ...
        expect(matchesText(match.uuid(), '3f2504e0-4f89-41d3-9a0c-0305e82c3301')).toBeTruthy();
        expect(matchesText(match.uuid(), 'not-a-uuid')).toBeFalsy();
    });
});
