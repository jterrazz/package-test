...
import { RENAMED_GROUND_DIRS } from '../../../model/chain/ground.js';
...
export const c13UnderscoredGround: LintRule = {
        ...
        return {
            Program(node: AstNode) {
                ...
                const probe = (parent: string, legacy: string): void => {
                    ...
                    seen.add(path);
                    context.report({
                        data: { ground: RENAMED_GROUND_DIRS[legacy] ?? legacy, legacy },
    ...
    meta: {
        ...
        messages: {
            notUnderscored:
                'The directory "{{legacy}}/" is a pre-14 ground name — rename it to "{{ground}}/" (`git mv {{legacy}} {{ground}}`). What a spec stands on carries the underscore; a spec\'s own folder never does, and no resolver reads the un-underscored name.',
        },
        type: 'problem',
    },
};
