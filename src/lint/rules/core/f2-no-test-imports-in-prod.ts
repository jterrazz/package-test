...
const TEST_FILE = /\.(?:(?:test|fixtures|specification)\.[cm]?[jt]sx?|spec\.[cm]?[jt]s)$/u;
...
export const f2NoTestImportsInProd: LintRule = {
    ...
    meta: {
        ...
        messages: {
            testImport:
                'Production code must not import the test artefact "{{source}}" — test imports are only legal from specs/, *.test.ts, *.fixtures.ts or *.specification.ts.',
        },
        type: 'problem',
    },
};
