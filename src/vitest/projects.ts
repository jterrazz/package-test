...
/**
 * The project's `wrap` module, as the DEV SERVER serves it: a page imports a
 * URL, never a filesystem path.
 */
function servedPath(wrap: string): string {
    const absolute = isAbsolute(wrap) ? wrap : resolve(process.cwd(), wrap);
    return `/${relative(process.cwd(), absolute).replaceAll('\\', '/')}`;
}

/**
 * What the page cannot read for itself, handed over as provided values: update
 * mode (a `process.env`/`argv` read), the project's clock, and the wrap module
 * as a URL.
 */
function providedToPage(options: ComponentProjectOptions): Record<string, boolean | string> {
    return {
        ...
        ...(options.wrap === undefined ? {} : { componentWrap: servedPath(options.wrap) }),
        update: updating(),
    };
}
