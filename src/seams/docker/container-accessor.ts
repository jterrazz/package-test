...
import type { FileAccessor } from '../../model/result/result.js';
...
export class ContainerAccessor {
    ...
    get stdout(): TextAccessor {
        ...
        return new TextAccessor(this.loadLogs(), 'stdout', this.testDir, {
    ...
    get stderr(): TextAccessor {
        ...
        return new TextAccessor(this.loadLogs(), 'stderr', this.testDir, {
            transform: this.transform,
        });
    }
    ...
    private containerExec(id: string, argv: string[]): CliOutput {
        // Piping stdio keeps a non-zero exit inside a CliOutput instead of throwing out.
        try {
