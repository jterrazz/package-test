...
import { dockerCommandSpec } from '../../setup/docker-command.specification.js';
...
describe('command — docker option (lazy container accessors)', () => {
    test.skipIf(!HAS_DOCKER)(
        ...
        async () => {
            ...
            {
                await using result = await dockerCommandSpec('spawn test-a')
                    .exec('spawn test-a')
                    .run();
    ...
    test.skipIf(!HAS_DOCKER)(
        ...
        async () => {
            ...
            await using result = await dockerCommandSpec('help').exec('help').run();
            expect(result.exitCode).toBe(0);
        },
        10_000,
    );
