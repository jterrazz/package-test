...
describe("cli — directory snapshot", () => {
  ...
  test("detects an extra file", async () => {
    ...
    const result = await cliSpec("scaffold extra").project("cli-app").exec("scaffold-extra").run();
  ...
  describe("update mode", () => {
    ...
    test("update: true writes the fixture", async () => {
      ...
      const result2 = await cliSpec("update verify").project("cli-app").exec("scaffold").run();
