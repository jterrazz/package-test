...
| `database is locked` on a cold sqlite cache / a suite pinned to `fileParallelism: false` for it | Fixed since 14.1.0 — the template build takes an exclusive lock and the losers WAIT. Upgrade, then drop the pin. [08](../../docs/08-services.md#one-worker-builds-the-others-wait) |
...
| `prismaSchema` green locally, "Could not find Prisma Schema" in CI                              | The cached template hides the cold path — `rm -rf .artifacts/vitest/sqlite` and re-run. [08](../../docs/08-services.md)                                                            |
