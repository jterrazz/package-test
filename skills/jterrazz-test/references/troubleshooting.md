...
| A `// Then -` marker ended up inside a `const a = …, b = …` chain after `lint:fix` | `one-var` in its `always` mode fused the declarations and swallowed the marker — a marker goes between STATEMENTS. [13](../../../docs/13-linting.md)    |
