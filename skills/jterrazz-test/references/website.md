...
Operative reference. Prose + examples: [docs/08-website.md](../../../docs/08-website.md). Assertions: [docs/14-assertions.md](../../../docs/14-assertions.md). Tokens: [references/tokens.md](tokens.md).
...
- Elements are **user-facing by construction (W2)**: `button(name)`, `link(name)`, `field(label)`, `heading(name)`, `content(text)`, the roles `dialog/status/table/row/listitem(name?)`, and `option(name)` named inside its field (`within(field('Channel'), option('LinkedIn'))`) — CSS/XPath is not expressible. `testId(id)` is the escape hatch and owes a `// testId: <what the element lacks>` on its line or the one above (W2, error). A name designates the accessible name WHOLE since 16.0; `{ exact: false }` is the opt-out, and a name that only matches in part warns once per descriptor during 16.0 and 16.1.
