...
The ecosystem's testing framework. One fluent builder, three target wrappers. Handles containers, working directories, and cleanup automatically.
...
- Edits under `tests/http/*.specification.ts` or `tests/cli/*.specification.ts`.
...
export const runner = await spec(
...
```typescript
// tests/http/users/users.e2e.test.ts
...
- **"fixture project not found"** -> `.project("name")` looks for `{root}/{name}/`, where `root` is set in the `spec(command(...), { root })` call. Confirm both.
...
- **grep returns nothing** -> use `result.grep(pattern)` (instance method), not the old standalone `grep()` import.
- **docker assertions fail** -> use `runner.docker(id)` (not the old `dockerContainer(id)` standalone function).
